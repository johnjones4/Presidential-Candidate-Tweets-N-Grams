...
exports.getCombinedMemberTallies = function(start,end,callback) {
  start = cleanDate(start,'0000-00-00');
  end = cleanDate(end,'9999-99-99');
  connection.query('SELECT t.Member_ID,t.Issue_ID,count(t.Count) as Count,m.Handle,i.Name FROM Tally t, Issue i, Member m WHERE t.Member_ID = m.ID AND t.Issue_ID = i.ID AND Day >= ? && Day <= ? GROUP BY t.Member_ID,t.Issue_ID', [start,end], function(err,results,fields) {
    callback(err,results);
  });
}
