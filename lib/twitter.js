...
class Twitter {
  ...
  scan(done) {
    ...
    async.waterfall([
      ...
      function(next) {
        async.series(
          _this.handleQueue.map(function(handle) {
            return function(next1) {
              _this.getNewTweets(handle,function(err) {
                next1(err);
              });
            }
          }),
        ...
        try {
          ...
          if (response && response.users) {
            ...
            response.users.forEach(function(user) {
              ...
              _handleQueue.push(user.screen_name);
            });
            _this.handles = _handles;
            _this.handleQueue = _handleQueue;
